...
public class TrainingPlanServiceImpl implements TrainingPlanService {
    ...
    public RecommendedActivityDto getTrainingPlan2(String email, double latitude, double longitude) {
        try {
            ApplicationUser user = userRepository.findUserByEmail(email);
            if (user == null) {
                throw new ResponseStatusException(HttpStatus.NOT_FOUND, "User not found");
            ...
            // 1) Build the 7-day plan ONCE; do NOT run any daily selector logic here
            TrainingPlan7dDto plan = trainingPlan7dService.buildNext7Days(
                    email,
                    latitude,
                    longitude,
                    false,
            ...
            PlannedDayDto day = pickDay(plan, today);

            // 2) Build athlete status for UI (does NOT influence planned workout)
            AthleteStatusDto athleteStatus = buildCurrentAthleteStatus(email, user, today);

            // 3) Project PlannedDayDto -> RecommendedActivityDto (compat)
            return project(day, athleteStatus);

        } catch (ResponseStatusException e) {
            throw e;
        } catch (Exception e) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to build training plan", e);
        }
    }

    private PlannedDayDto pickDay(TrainingPlan7dDto plan, LocalDate date) {
        if (plan == null || plan.getDays() == null || plan.getDays().isEmpty()) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "7-day plan returned no days");
        }
    ...
    public record PlanContext(
            ...
            LocalDate today,
            double tsbToday,
            int readinessScoreToday,
            double injuryIndex,
            List<Injuries> allInjuries,
            List<ViewInjuryDto> activeInjuries,
            WeatherResponse weatherNow,
            CompactWeatherDto compactWeatherNow,
            RouteDto route
    ) {
    }
}
